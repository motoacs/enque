...
func TestParseProgress_Normal(t *testing.T) {
	...
	line := "[53.2%] 1234 frames: 245.67 fps, 12345 kbps, remain 0:01:23, GPU 45%, VE 78%, VD 12%"
...
func TestParseProgress_RealOutput(t *testing.T) {
	a := &NVEncCAdapter{}

	line := "[0.6%] 39 frames: 41.85 fps, 20744 kbps, remain 0:02:46, GPU 0%, VE 33%, VD 12%, est out size 288.6MB"
	prog := a.ParseProgress(line)

	if prog.Percent == nil || *prog.Percent != 0.6 {
		t.Errorf("percent=%v, want 0.6", prog.Percent)
	}
	if prog.FPS == nil || *prog.FPS != 41.85 {
		t.Errorf("fps=%v, want 41.85", prog.FPS)
	}
	if prog.BitrateKbps == nil || *prog.BitrateKbps != 20744 {
		t.Errorf("bitrate=%v, want 20744", prog.BitrateKbps)
	}
	if prog.ETASec == nil || *prog.ETASec != 166 {
		t.Errorf("eta=%v, want 166", prog.ETASec)
	}
}

func TestParseProgress_LegacyKbSlash(t *testing.T) {
	a := &NVEncCAdapter{}
...
func TestParseProgress_100Percent(t *testing.T) {
	...
	line := "[100.0%] 5000 frames: 300.00 fps, 8765 kbps, remain 0:00:00"
	prog := a.ParseProgress(line)
...
func TestParseProgress_NoRemain(t *testing.T) {
	...
	line := "[10.5%] 100 frames: 50.00 fps, 5000 kbps"
	prog := a.ParseProgress(line)
...
func TestParseProgress_HighBitrate(t *testing.T) {
	a := &NVEncCAdapter{}
	line := "[75.3%] 3750 frames: 500.12 fps, 123456 kbps, remain 0:00:30"
	prog := a.ParseProgress(line)
...
func TestParseProgress_LongETA(t *testing.T) {
	a := &NVEncCAdapter{}
	line := "[1.0%] 50 frames: 10.00 fps, 5000 kbps, remain 2:30:45"
	prog := a.ParseProgress(line)
