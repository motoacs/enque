...
Enqueは、rigaya氏のNVEncC（NVEncC64.exe）を初期ターゲットとするWindows用GUIラッパーアプリケーションである。カメラで撮影した動画や各種映像ファイルを、ドラッグ＆ドロップで追加し、NVIDIAのハードウェアエンコーダ（NVENC）を用いて一括エンコードする。RTX 5090の3基NVENCエンジンを活用した高速エンコードに対応する。

対象ユーザーはエンコーダCLI（NVEncC/QSVEncC/ffmpeg）のオプションを理解している上級者であり、GUIは日常のバッチエンコード作業を効率化するためのプロの道具として設計する。主要なオプションをGUIウィジェットで提供しつつ、任意のコマンドラインオプションをテキスト入力で追加できる柔軟性を持つ。初期リリースはNVEncCを実装対象とし、将来のQSVEncC/ffmpeg拡張は本計画書の拡張基盤設計に従って追加する。
...
| データ永続化 | JSONファイル |
| 外部依存 | NVEncC64.exe 8.x以降（v1必須）、QSVEncC64.exe（将来拡張）、ffmpeg.exe / ffprobe.exe（任意） |
...
- v1 実装対象: NVEncC 8.x系
- 将来拡張対象: QSVEncC（rigaya）/ ffmpeg
- 拡張時の再利用対象: キュー管理、実行制御、出力名解決、ログ保存、Wails API/Event 契約
...
**US-03**: ユーザーとして、「エンコード開始」ボタンを押して、キュー内の全ファイル（待機中を含む）の状態を一覧で確認しながら、各ファイルの進捗をリアルタイムで確認したい。
...
**US-11**: ユーザーとして、エンコード中にキューの停止（次のジョブを開始しない）や、実行中ジョブのキャンセル、待機中ジョブの個別スキップを行いたい。
...
**US-17**: ユーザーとして、将来QSVEncC/ffmpeg対応が追加されても、キュー管理・実行制御・ログ運用の使い方を変えずに利用したい。
...
- エンコード実行中はキュー編集をロックする（ただし待機中ジョブの個別スキップは可能）
- エンコード開始時に全ジョブを「待機中」としてジョブリストに初期表示する。バックエンドが処理を開始していないジョブも含め、キュー全体の状態を常に把握できるようにする
- エンコード中、待機中のジョブにはホバー時にスキップボタンを表示する。スキップされたジョブはワーカーが取り出した際に処理をスキップする（`SkipJob` API）
...
- プロファイルは `encoder_type`（`nvencc` / `qsvenc` / `ffmpeg`）を持ち、ツール固有設定は `encoder_options` に分離して保持する（v1プリセットは `nvencc` のみ）
...
GUIに載せていない選択中エンコーダのオプションを、テキスト入力欄にそのまま記述できる。例（NVEncC）:

```
--gop-len 300 --lookahead-level 3 --vpp-nlmeans sigma=0.005
```

入力内容は第1層のGUI設定で生成されたオプション列の末尾に追加される。競合時の優先順は「後方優先（later wins）」を基本とし、最終解釈は各エンコーダの仕様に従う。初期実装（NVEncC）では同一オプションがGUI側とテキスト側の両方にある場合、テキスト側が上書きする。

### F-05: コマンドラインプレビュー

第1層と第2層を合成した最終的なエンコーダコマンドラインを常時表示する。プロファイルや設定の変更に連動してリアルタイム更新される。クリップボードへのコピーボタンを備える。表示は参考用であり、コピペ実行を完全に保証するものではない。
...
- Goはジョブを実行する。同時実行数の設定に従い、1〜N個のエンコーダプロセスを並列に起動する（詳細は6.3節）。v1で実際に起動するのはNVEncCのみ
- 各ジョブの出力パス確定はジョブ開始直前にmutex下で行い、並列時の名前衝突を防止する。エンコードは一時ファイル `{dir}/{name}.{short_id}.tmp.{ext}` に出力し（最終拡張子を維持し、各エンコーダのコンテナ判定を壊さない）、成功後に最終ファイル名 `{dir}/{name}.{ext}` にリネーム（原子的）した上でファイル日時を復元する。{short_id} は8文字のランダム英数字とする。エンコード失敗時の一時ファイルはデフォルトで削除する。アプリ設定 `keep_failed_temp`（デフォルトfalse）がtrueの場合は削除せず残す（問題調査用）
- 選択中エンコーダのstderr出力をリアルタイムに解析し、進捗（パーセント、fps、ビットレート、残り時間）をWailsイベントでフロントに通知する。パースが失敗した場合は進捗を「不明」として扱い、エンコード自体は続行する。ログウィンドウには常に生の出力を表示する
- ジョブごとの進捗バーおよび全体の進捗（完了数/全数）を表示
- UI上に以下の実行状態を表示する: 同時実行数（現在稼働中/最大）、各ジョブの開始時刻・経過時間、終了コード
- ログウィンドウにエンコーダ出力をリアルタイム表示（フロント側はリングバッファで最新N行を保持）
- ジョブごとに以下をファイルに保存する:
  - `{job_id}.json`: 下記6.11節のスキーマに従うジョブ実行記録
  - `{job_id}.stderr.log`: エンコーダのstderr出力全文
- アプリ起動時に、前回のクラッシュ等で残存した一時ファイル（`*.tmp.{ext}` パターン）をスキャンし、検出された場合はユーザーに削除を提示する

### F-08: エンコード実行制御

- **停止（Graceful Stop）**: 「残りをスキップ」ボタンで次のジョブの開始を抑止する。実行中のジョブは完了まで走る。ツールチップで「現在実行中のジョブ完了後、残りのジョブをスキップします」と説明する
- **中止（Abort）**: 「すべて強制終了」ボタンで実行中の全ジョブを強制終了し、キュー処理を即座に終了する。ツールチップで「実行中のジョブを含めすべてを即座に強制終了します」と説明する
- **ジョブの個別スキップ**: エンコード中、待機状態のジョブをリストからスキップ指定できる。スキップされたジョブはワーカーが取り出した際に処理をスキップする
...
- ハング検出: 以下の2段階で判定する
  - **出力タイムアウト** (`no_output_timeout_sec`, デフォルト600秒): stderrから何らかの行を最後に受信してからの経過時間。エンコーダプロセスが完全に無応答になった場合を検出する
...
`NVEncC64.exe --check-device` および `--check-features` の実行結果を表示するダイアログ。搭載GPU名、NVENCエンジン数、対応コーデック・機能を確認できる。将来のQSVEncC拡張時は同等情報を取得する別コマンドをadapter側で定義する。

### F-12: 外部ツールパス設定

起動時に同一フォルダ → PATH の順で NVEncC64.exe を検出する。検索候補は `NVEncC64.exe`, `NVEncC.exe` の複数名。見つからない場合はパス設定ダイアログを表示する。設定画面からいつでも変更可能。

QSVEncC拡張に備え、`QSVEncC64.exe` / `QSVEncC.exe` のパス設定欄も設ける（任意）。v1では未検出でもエラーにしない。
...
- Backendのエンコーダ実装は `registry + adapter interface` で管理し、`command build` / `progress parse` / `capability判定` をadapterに閉じ込める
- `queue manager`、`output_resolver`、`timeout_guard`、`job logging`、`Wails API/Event` はエンコーダ非依存の共通実装とする
- フロントエンドは「共通設定UI（キュー・出力・実行制御）」と「エンコーダ固有設定UI（プロファイル編集）」を分離する
- プロファイルは `encoder_type` と `encoder_options` を持ち、ツールごとに独立した設定スキーマへ切り替え可能にする
- v1で同梱・編集可能なプリセットは `nvencc` のみとし、QSVEncC/ffmpegは拡張実装追加時に有効化する
...
| 外部依存 | NVEncC64.exe 8.x以降（v1必須）。QSVEncC64.exe（将来拡張）、ffmpeg.exe / ffprobe.exe（任意）。いずれもアプリに内蔵せず、同一フォルダまたはPATHから検出。READMEに入手先を明記する |
...
- エンコード実行中のジョブキュー編集は不可（ロック）。ただし待機中ジョブの個別スキップは可能
- 一時停止（プロセスサスペンド）機能は提供しない
- ジョブ個別のプロファイル割り当ては初期バージョンでは実装しない（データ構造は拡張を想定）
- NVEncC 7.x以前はサポート外
- 初期バージョンではエンコーダ実装はNVEncCのみ。QSVEncC/ffmpegは拡張ポイントのみ定義する
...
└──────────────────────┬───────────────────────────────────┘
                       │ os/exec
                       ▼
              selected encoder binary ×N (v1: NVEncC64.exe)
```
...
**エンコード中**: Go側が選択中エンコーダのプロセスを管理し、進捗をWailsイベントでフロントに通知する。フロントの `encodeStore` はイベントを受信して進捗UIを更新する。キューの停止・ジョブのキャンセル操作はフロントからGoのメソッドを呼ぶ。

### 6.3 並列実行戦略

NVEncC利用時は、RTX 5090の3基NVENCエンジンを活用するため、2つのレイヤで並列化を制御する。
...
QSVEncC/ffmpeg拡張時は、レイヤ1（プロセス並列）は共通のまま維持し、レイヤ2の可否・具体オプションは各adapterが定義する。
...
**並列実行時の一時ファイル方式**: エンコードは `{dir}/{name}.{short_id}.tmp.{ext}` の一時ファイルに出力する（最終拡張子を維持する）。{short_id} は8文字のランダム英数字とする。エンコード成功後に最終パスにリネームし、ファイル日時を復元する。失敗時は一時ファイルを削除する（keep_failed_temp=true の場合は保持）。これにより、途中生成の不完全なファイルが最終出力先に残ることを防ぐ。

### 6.4 コマンドライン構築

`encoder/registry.go` が `profile.encoder_type` に対応するadapterを解決し、adapter実装が引数列を生成する。共通の構築順序は以下の通り。

1. adapter前置オプション（例: デコーダ指定）
2. 入力指定（例: `-i {input_path}`）
3. 第1層GUIオプション（encoder固有）
4. 第2層カスタムオプション（後方優先）
5. 出力指定（例: `-o {output_path}`）

v1の `nvencc` adapter では、既存契約（`--avhw/--avsw` -> `-i` -> `-c` -> GUI -> custom -> `-o`）を維持する。
...
進捗パーサーはadapterごとに実装する。v1の `nvencc` adapter はstderrに進捗を `\r` で上書きしながら出力するため、Go側で `\r` / `\n` をデリミタとするカスタムスキャナーで行単位に読み取り、正規表現で進捗情報を抽出する。

**開発初期段階で実施すべき事項（nvencc）**: NVEncC64.exeの実際の出力を、以下の各パターンでファイルに記録し、パーサーの仕様とテストデータを確定する。

- 通常エンコード（H.264 / HEVC / AV1 各コーデック）
- `--split-enc` 使用時
- `--parallel` 使用時
- プロセス並列（複数エンコーダプロセス同時実行）時の各プロセスの出力
- エラー発生時（入力ファイル不正、GPU非対応、コーデック非対応オプション等）
- 入力ファイルの長さが不明な場合

パースが失敗した場合（出力形式変更等）は進捗を「不明」として扱い、エンコード自体は続行する。ログウィンドウには常にパース前の生の出力を表示する。
...
Goの `os/exec` で選択されたエンコーダ実行ファイルを起動する。

**プロセスツリー管理**: エンコーダプロセスの起動時にWindows Job Object を作成・割り当てる。
...
- `AssignProcessToJobObject` でエンコーダプロセスを割り当て
...
**デコーダフォールバック**: `nvencc` adapterにのみ適用する。`--avhw` で失敗した場合（終了コード非0）に `--avsw` に切り替えて1回だけ自動再試行する。アプリ設定でON/OFF可能、デフォルトOFF。再試行時はログに「avhw失敗、avswで再試行」を記録する。
...
**QSVEncC64.exe**: 同様の順序で検出する。候補は `QSVEncC64.exe`, `QSVEncC.exe`。v1では未検出でもエラーにはしない（拡張準備情報として表示のみ）。
...
Profile {
    ...
    encoder_options: object (エンコーダ固有設定領域、JSON object)

    // 以下は nvencc 向け設定（encoder_type=nvencc で使用）
    // 映像基本
    codec:       "h264" | "hevc" | "av1"
...
AppConfig {
    ...
    qsvenc_path:             string  (空文字 = 自動検出、任意)
    ...
    output_name_template:    string  (デフォルト: "{name}_encoded.{ext}")
...
**ユニットテスト**: `profile/manager.go` のマイグレーション。旧バージョンのプロファイルJSONを読み込み、`encoder_type` / `encoder_options` を含む現行構造に変換できることを検証する。
...
JobRecord {
    ...
    // エンコーダ実行情報
    encoder_type:         "nvencc" | "qsvenc" | "ffmpeg"
    encoder_path:         string  (使用した実行ファイルの絶対パス)
    ...
    retry_applied:        bool    (adapter定義の再試行が発生したか)
    retry_detail:         string  (例: "nvencc: avhw->avsw", なければ空)
}
```
